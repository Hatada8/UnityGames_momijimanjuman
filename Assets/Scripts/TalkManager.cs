...
public class TalkManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            new GameObject("GameFlagManager").AddComponent<GameFlagManager>();
        }
    ...
    void TryGiveItem()
    {
        ...
        if(gameObject.name == "satooshi" || gameObject.name == "higashi")
        {
            NPCItemSpawner spawner = GetComponent<NPCItemSpawner>();
               // 通常アイテム（もみじまんじゅう）
            if(currentItem == requiredItem)
            {
                if(itemGiven) return;
                ItemManager.Instance.RemoveCurrentItem(spawner);
    ...
    void StartDialogue()
    {
        //ひよこに話しかける時、桃太郎と話してたならシーン移動。
        if (gameObject.name == "hiyoko")
{
    if (GameFlagManager.Instance != null &&
        GameFlagManager.Instance.HasTalkedToMomotaro())
    {
        MySceneManager.Instance.LoadScene("Scene2to7");
        return;
    }
}
        isTalking = true;
    ...
    IEnumerator TypeGiftSentence2(string sentence)
{
    ...
    void EndDialogue()
    {
        ...
        //桃太郎とはなしたときにフラグをtrueにする。
        if(gameObject.name == "Momo")
        {
            GameFlagManager.Instance.SetTalkedToMomotaro();
        }
        if (messageWindow != null) 
        {
            messageWindow.SetActive(false);
